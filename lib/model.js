...
function Model(obj) {
  ...
  this.attributes = utils.extend({id: null}, defaults, obj || {});
  this.initialize(obj);
};
...
/**
 *
 * Concatenates dbName, name and id. Thus: <dbName>:<modelName>:<modelId>
 */
Model.key = Model.prototype.key = function(name, id) {
  return this.database.config.dbName + ":" + name + ":" + id;
};
 ...
 * @param {Object} client
 */
function ModelWrapper(database, client) {
  Model.prototype.client = Model.client = client;
  Model.prototype.database = Model.database = database;
  return Model;
};
